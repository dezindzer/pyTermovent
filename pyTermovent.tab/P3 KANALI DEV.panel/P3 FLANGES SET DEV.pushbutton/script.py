...
def NadjiPrirubniceSET(element):
    ...
    for Kon in konektori:  #PROLAZI SE KROZ SVAKI KONEKTOR ELEMENTA
        ...
        if parW and TipSeta:
            try:
                Param=kan.GetParameters(parW.Name +' Flange')[0]
                Param.Set(TipSeta)
...
if __name__=='__main__':   #TEST PROGRAM KOJI TESTIRA FUNKCIJU NadjiPrirubnice NA POJEDINACNOM ELEMENTU A ZATIM PRAVI PRIRPREMLJENU LISTU ZA IZLAZ I ISPIS.
    ...
    Rezultat=[]
    Problematicni=[]
    TrG=TransactionGroup(doc, "P3 - FLANGES SET")  #--TRANSAKCIJA
    TrG.Start()   #--TRANSAKCIJA
    ...
    #else:
      #  TrG.RollBack()
       # print('Елемент: ID(' + str(Problematicni) +') није добро повезан (СТАВИТИ ЧЕП АКО ЈЕ ОСТАЈЕ НЕПОВЕЗАН) или је није из P3 програма производа' )

...
def NapraviP3801(Elementi):
    ...
    parametri801CategoryDucts=['Width','Height','Length']
    parametri801CategoryDuctFittings={'P3 - A Width_1':'P3_Width', 'P3 - B Depth_1':'P3_Height','P3 - DuctLength':'P3_Length'}
    listaKanala=[]
    for element in Elementi:
        debljinaMaterijala=element.get_Parameter(BuiltInParameter.RBS_REFERENCE_INSULATION_THICKNESS).AsInteger()
        Mark=element.get_Parameter(BuiltInParameter.ALL_MODEL_MARK).AsString()
        ...
        if element.Category.Name=='Ducts':
            for j in parametri801CategoryDucts:
                parametri['P3_'+j]=PretvoriJedinicu(element.GetParameters(j)[0])
        elif element.Category.Name=='Duct Fittings':
            for j in parametri801CategoryDuctFittings:
                parametri[parametri801CategoryDuctFittings[j]]=PretvoriJedinicu(element.GetParameters(j)[0])        
        else:
            return False

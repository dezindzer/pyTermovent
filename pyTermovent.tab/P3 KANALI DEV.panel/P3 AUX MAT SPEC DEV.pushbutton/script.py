                ...
                if Par.Owner.Category.Name == 'Duct Fittings':
                    try:
                        ...
                        if KODelementa == 812 and not Kon.GetMEPConnectorInfo().IsPrimary:
                            ...
                            PRIRUBNICElista.extend(CitacKonektoraF_UBOD(Kon))
                            TipSeta='2'
                    ...
                    exit(1)
        if parW and TipSeta:
            PrirubnickiSetovi.append([parW.Name+' Flange',TipSeta])          
                    
    return PRIRUBNICElista,PrirubnickiSetovi
...
if __name__=='__main__':   #TEST PROGRAM KOJI TESTIRA FUNKCIJU NadjiPrirubnice NA POJEDINACNOM ELEMENTU A ZATIM PRAVI PRIRPREMLJENU LISTU ZA IZLAZ I ISPIS.
    ...
    SelektovaniKanali = [el for el in FilteredElementCollector(doc, uidoc.Selection.GetElementIds()).OfCategory(BuiltInCategory.OST_DuctFitting).ToElements() ]
    PrirubniceS=[]
    PrirubniceU=[]
    PrirubniceF=[]
    PrirubniceB=[]
    Tr=Transaction(doc, "P3 - FLANGES CALCULATION")  #--TRANSAKCIJA
    Tr.Start()   #--TRANSAKCIJA
    for kan in SelektovaniKanali: 
        p=NadjiPrirubnice(kan)
        if p:
            listaPrirubnica=p[0]
            Set=p[1]
            for i in Set:
                Param=kan.GetParameters(i[0])[0]
                Param.Set(i[1])
            Tr.Commit()
            for j in listaPrirubnica:  #PRIRUBNICE SE RAYVRSTAVAJU PO TIPU U PRIRPADAJUCE LISTE
            ...
            Tr.RollBack()
            print('Елемент: ID(' + str(kan.Id) +') није добро повезан (СТАВИТИ ЧЕП АКО ЈЕ ОСТАЈЕ НЕПОВЕЗАН)' )
